...
namespace iai_naive_kinematics_sim {
	...
	bool ExpressionTree::parseYAML(const string &path, 
		...
		if (root["fake-controllers"]) {
			const YAML::Node& fakers = root["fake-controllers"];
			if (!fakers.IsSequence()) {
				cerr << "Fake controllers node needs to be a sequence!" << endl;
				return false;
			}

			for (size_t i = 0; i < fakers.size(); i++)
			{
				if (!fakers[i].IsMap()) {
					cerr << "Fake controllers need to be a map! Node: " << endl << fakers[i] << endl;
					return false;
				}

				try {
					string jointName = fakers[i].begin()->first.as<string>();
					if (sim->hasJoint(jointName))
					{
						size_t idx = sim->getJointIndex(jointName);
						if (fakers[i].begin()->second["position"]) {
							Expression<double>* posExp = parseDoubleExpr(fakers[i].begin()->second["position"]);
							if (posExp) {
								posExprs[idx] = posExp;
							} else {
								cerr << "Parsing of position expression for joint '" << jointName << "' failed! Node: " << endl << fakers[i].begin()->second << endl;
								return false;
							}
						}

						if (fakers[i].begin()->second["velocitiy"]) {
							Expression<double>* velExp = parseDoubleExpr(fakers[i].begin()->second["velocitiy"]);
							if (velExp) {
								velExprs[idx] = velExp;
							} else {
								cerr << "Parsing of velocitiy expression for joint '" << jointName << "' failed! Node: " << endl << fakers[i].begin()->second << endl;
								return false;
							}
						}

						if (fakers[i].begin()->second["effort"]) {
							Expression<double>* effExp = parseDoubleExpr(fakers[i].begin()->second["effort"]);
							if (effExp) {
								effExprs[idx] = effExp;
							} else {
								cerr << "Parsing of effort expression for joint '" << jointName << "' failed! Node: " << endl << fakers[i].begin()->second << endl;
								return false;
							}
						}
					} else {
					...
					cerr << "Unable to parse node as string! Node: " << endl << fakers[i] << endl;
				}
			}
		} else
			cerr << "Node 'fake-controllers' could not be found as child of YAML root. Root: " << root << endl;
